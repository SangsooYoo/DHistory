...
public class SolutionL3010 {
    ...
    public String solution(int n, int t, int m, String[] timetable) {
        String answer = "";
        Queue<Integer> crews = new PriorityQueue<>();
        ...
        int busTime = 9 * 60; // 9시
        int latestTime = busTime;
        for (int i = 0; i < n; i++) {
            while (!crews.isEmpty() && crews.peek() <= busTime) {
                waiting.add(crews.poll());
            }
            System.out.println("watigin=" + waiting.toString());
            if (waiting.size() < m) {
                latestTime = busTime;
                for (int j = 0; j < m && !waiting.isEmpty(); j++) {
                    waiting.poll();
                }
                System.out.println("latestTime2=" + latestTime);
            } else {
                for (int j = 0; j < m && !waiting.isEmpty(); j++) {
                    System.out.println("peak=" + waiting.peek());
                    latestTime = waiting.poll() - 1;
                }
                System.out.println("latestTime1=" + latestTime);
            }

            busTime = busTime + t;
        }

        answer = String.format("%02d:%02d", Math.floorDiv(latestTime, 60), Math.floorMod(latestTime, 60));
        return answer;
    }
}
